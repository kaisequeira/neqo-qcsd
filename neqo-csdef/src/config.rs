...
impl QcsdConfig {
    ...
    pub fn from_toml_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        config.resolve_static_schedule(path);
        Ok(config)
    }
    ...
    pub fn from_toml(input: &str) -> Result<Self> {
        let mut config: Self = match toml::from_str(input) {
            Ok(config) => config,
    ...
    pub fn from_legacy_toml(input: &str) -> Result<Self> {
        ...
        let config = Self {
            ...
            use_empty_resources: flow.use_empty_resources,
    ...
    fn resolve_static_schedule(&mut self, config_path: &Path) {
        let DefenseConfig::Static { schedule, .. } = &mut self.defense else {
            return;
        };
        let schedule_path = Path::new(schedule);
        if schedule_path.is_absolute() {
            return;
        }
        let relative = config_path
            ...
            .join(schedule_path);
        // Version-one configurations historically resolved paths from the
        // process working directory. Prefer the owning configuration when it
        // exists, while retaining that fallback for legacy inputs.
        if relative.exists() || !schedule_path.exists() {
            *schedule = relative.to_string_lossy().into_owned();
        }
    }
}
